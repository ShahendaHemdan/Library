<?php if (isset($_POST['search-for']) && isset($_POST['search-key'])) { ?>
  <!DOCTYPE html>
  <html lang="en">
  ...
  <body>
    ...
    <section style="padding: 10px;">
      ...
      if ($db->connect()) {
        $key = val($_POST['search-key']);
        $sql = "SELECT * FROM book 
        WHERE book.category LIKE '%$key%' OR
        book.name LIKE '%$key%' OR
        book.writer LIKE '%$key%';";
